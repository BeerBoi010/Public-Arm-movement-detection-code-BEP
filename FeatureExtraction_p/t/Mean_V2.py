...
def Mean_all_subjects(sampling_window, min_periods):
    ...
    for subject in subjects:
        ...
        for imu_location in imu_locations:
            ...
            combined_data_patient.append({'acc_mean': mean_acc, 'rot_mean': mean_rot})

        combined_data_all_subjects.append(combined_data_patient)


    overall_mean_data = {'acc_mean': pd.concat([data['acc_mean'] for subject_data in combined_data_all_subjects for data in subject_data], axis=1).mean(axis=1),
                         'rot_mean': pd.concat([data['rot_mean'] for subject_data in combined_data_all_subjects for data in subject_data], axis=1).mean(axis=1)}

    return overall_mean_data
